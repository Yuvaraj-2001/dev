...
function SelectTopics({topicSend}){
    ...
    if(isLoading){
        ...
        content = data.map((n)=> <div className={ activeTopic.id === n.id ? `bg-purple-400 text-slate-200 p-2` : `text-slate-200 p-2`} onClick={()=>redirect(n)}><span className="hover:text-gray-300">{n.name}</span></div>)  
    }
